...
const DeliveryConfig = (props) => {
    ...
    const date_wise_day_name = async () => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const dayname=moment(datet).format('dddd').substring(0,3).toLowerCase();;
    ...
    const set_dayname =async (obj) => {
        ...
        setSelectedDay(obj)
        dispatch(setOrderConf({ ...orderconfg,
            schedule_date:obj.datetimestamp
        }))
...
//                         })
                    ...
                    {
                        delivery_config.length ? delivery_config.map(({id,type,time_take})=>{
                            if(type === 'now'){
                                return (<TabPanel>
                                    <div>
                                        <div className="text-center mb-2 py-2">
                                            ...
                                            <h6 className="font-weight-bold text-dark">Your order will be deliver in {time_take} mins</h6>
                                        </div>
                                        {/* <div className="p-3">
                            ...
                            <div className="osahan-order_address">
                                ...
                                <div className="schedule">
                                    <ul className="nav nav-tabs justify-content-center nav-fill" id="myTab" role="tablist">
                                    {
                                        ...
                                        calenderdates.map(({id,date,dayname,datetimestamp})=>{
                                            return (
                                                <li key={id} onClick={()=>set_dayname({id,date,dayname,datetimestamp})} className="nav-item" role="presentation" style={{cursor:'pointer'}}>
                                                    <span className={`nav-link ${selected_day.datetimestamp === datetimestamp ? active_class  : ''} text-dark`}  aria-selected="true">
                                                        <p className="mb-0 font-weight-bold">{dayname.toUpperCase()}</p>
...

                                    {
                                        ...
                                        timeslotobj.map(({start,end})=>{
                                            return (
                                                <div style={{cursor:'pointer'}} onClick={()=>select_time_slot(start+'-'+end)} className="tab-content filter bg-white" >
                                                    <div className="tab-pane fade show active" id={start+'-'+end} role="tabpanel" aria-labelledby="mon-tab">
                                                        <div className="custom-control border-bottom px-0 custom-radio">
                                                            <input className="custom-control-input" type="radio" name="time_slot" id={start+'-'+end}  value={start+'-'+end}  checked={selectedTimeSlot==start+'-'+end ? true : false}  />
                                                            <label className="custom-control-label py-3 w-100 px-3" for={start+'-'+end} >
                                                                <i className="icofont-clock-time mr-2"></i> {start} - {end}
                                                            </label>
                                                        </div>
                                                    </div>
                                                </div>

...
import { get_blogs, get_blog_by_category, get_blog_categories } from "../../server/api";
import "./Blog.css";
    ...
    <div className="entry-item lifestyle shopping col-sm-6 col-lg-4">
              <article className="entry entry-grid text-center">
                <figure className="entry-media">
                  <Link
                  to={{
                    pathname:'/blog-details/'+props.title.replace(/\s+/g, "-")+'/'+props.id
                  }}
                ...
                <div className="entry-body">
                  ...
                  >{moment(props.created_at).format('MMM DD, YYYY')}</Link>
                    <span className="meta-separator">|</span>
                    <Link
                  to={{
                    pathname:'/blog-details/'+props.title.replace(/\s+/g, "-")+'/'+props.id
                  ...
                  <div className="entry-content">
                    {props.description ? ReactHtmlParser(htmlSubstring(props.description,200)) : ""}
                  {/* </ShowMoreText> */}
                    <Link
                    className="read-more"
                  to={{
        ...
        <div className="container">
          <nav className="blog-nav">
            <ul className="menu-cat entry-filter justify-content-center">
              ...
              <li className={active_class==='all'?'active':''} onClick={()=>fetch_blog_by_category(0,'all')}  >
              ...
              </li>
              {
               categories && categories.map((dt)=>{
                 return (
                    <li className={active_class===dt.name?'active':''}  onClick={()=>fetch_blog_by_category(dt.id,dt.name)}>
                      <a style={{cursor:"pointer"}} >
                        {dt.name}<span>{dt.total_blogs}</span>
                      </a>
                    </li>
                 )
               })

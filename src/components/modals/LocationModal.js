...
import { check_if_service_location, fetch_areas, fetch_locations, fetch_location_by_id } from "../server/api";
        ...
        const setlc = !props.isGeolocationAvailable ? (
            ...
            if(setlc.status){
				...
				if (check_service_location.status) {
                    ...
                    setErrormessage({
                        message:"Sorry we are not providing service in your area, Please choose other location from the list",
                        class:"danger"
                    })
